...
module.exports = exports = function (router) {
  ...
  router.param('uiScreen', function(req, res, next, id) {
    ...
    query.exec(function (err, uiScreen){
      ...
      if (!uiScreen) { return next(new Error("can't find screen")); }
  ...
  router.put('/:uiScreen', function(req, res, next) {
    console.log(req.uiScreen);
    req.uiScreen = req.body;    
    console.log(req.body);
    var uiScreen = new UIScreen(req.uiScreen);
    // uiScreen.save(function(err, jsonData){
    //   if(err){ return next(err); }
